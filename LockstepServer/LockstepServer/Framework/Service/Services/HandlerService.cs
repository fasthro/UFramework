...
{
    public class HandlerService : BaseService, IHandlerService
    {
        ...
        public void OnReceive(NetPeer peer, int cmd, int session, NetworkProcessLayer layer, byte[] data)
        {
            ...
            if (handlerDict.TryGetValue(cmd, out handler))
            {
                try
                {
                    ...
                    {
                        _networkService.Send(peer, handler.cmd, handler.session, handler.layer, handler.responseMessage);
                    }
                    handler.DoAction();
                }
                catch (Exception ex)
                {
                    ...
                    _networkService.Send(peer, NetwokCmd.SERVER_EXCEPTION, 0, NetworkProcessLayer.All, s2c);
